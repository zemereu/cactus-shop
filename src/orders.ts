...
function setComeziTab(tab: 'history' | 'lookup') {
    if (!tabHistory || !tabLookup || !historyPanel || !lookupPanel) return;
...
if (tabHistory) tabHistory.addEventListener('click', () => setComeziTab('history'));
if (tabLookup) tabLookup.addEventListener('click', () => setComeziTab('lookup'));

// --- Status badge color ---
function comenziStatusColor(status: string): string {
    if (status === 'Livrată') return '#2f694b';
    if (status === 'Expediată') return '#1976d2';
    if (status === 'Plătită - în pregătire') return '#FF9800';
    return '#d32f2f'; // Neplătită
}

function comenziStatusIcon(status: string): string {
    if (status === 'Livrată') return 'fa-circle-check';
    if (status === 'Expediată') return 'fa-truck';
    if (status === 'Plătită - în pregătire') return 'fa-box';
...
if (comenziLookupBtn) {
    comenziLookupBtn.addEventListener('click', async () => {
        const tokenInput = (document.getElementById('lookup-order-token') as HTMLInputElement).value.trim();
