            ...
            if (list) {
                ...
                for (const main of MAIN_CATEGORIES) {
                    ...
                    <div style="width: 100%; margin-bottom: 15px;">
                        ...
                        : subcats.map(cat => `<li style="background: #fdf2b8; border: 1px solid #2f694b; color: #2f694b; padding: 8px 12px; border-radius: 20px; display: flex; align-items: center; gap: 10px; font-weight: bold;">
                                        ${escapeHtml(cat.name)}
                ...
                document.querySelectorAll('.delete-cat-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (confirm("Sigur ștergi această subcategorie?")) {
                            yield authFetch(`${API_BASE}/api/categories/${id}`, {
                                method: 'DELETE',
                            });
                            fetchAdminCategories();
                        }
...
function updateCactusCategoryDropdown(categories) {
    const mainSelect = document.getElementById('new-cactus-main-category');
...
if (mainCategorySelect) {
    mainCategorySelect.addEventListener('change', () => __awaiter(void 0, void 0, void 0, function* () {
        const response = yield authFetch(`${API_BASE}/api/categories`);
        const categories = yield response.json();
        updateCactusCategoryDropdown(categories);
    }));
...
if (addCategoryBtn) {
    addCategoryBtn.addEventListener('click', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const mainSelect = document.getElementById('new-category-main');
        if (!nameInput.value.trim() || !mainSelect)
            return;
...
function fetchAdminCacti() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield authFetch(`${API_BASE}/api/cacti/all`, {});
            const cacti = yield response.json();
            ...
            document.querySelectorAll('.save-edit-btn').forEach(button => {
                button.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const updated = {
                        ...
                        price: Number(panel.querySelector('.edit-price').value),
                        stock: Number(panel.querySelector('.edit-stock').value) || 0,
                        description: panel.querySelector('.edit-desc').value.trim(),
                        imageUrl: panel.querySelector('.edit-image').value.trim(),
                        productType: panel.querySelector('.edit-product-type').value,
                        mainCategory: panel.querySelector('.edit-main-category').value,
                        category: panel.querySelector('.edit-category').value
                    };
                    const response = yield authFetch(`${API_BASE}/api/cacti/${id}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
            ...
            document.querySelectorAll('.delete-cactus-btn').forEach(button => {
                button.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (confirm("Ești sigur că vrei să dezactivezi acest produs? Nu va mai apărea în magazin.")) {
                        yield authFetch(`${API_BASE}/api/cacti/${cactusId}`, {
                            method: 'DELETE',
                        });
                        fetchAdminCacti();
                    }
            ...
            document.querySelectorAll('.reactivate-cactus-btn').forEach(button => {
                button.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
                    const btn = event.target;
                    const cactusId = Number(btn.getAttribute('data-id'));
                    yield authFetch(`${API_BASE}/api/cacti/${cactusId}/reactivate`, {
                        method: 'PUT',
                    });
                    fetchAdminCacti();
                }));
            ...
            document.querySelectorAll('.hard-delete-btn').forEach(button => {
                button.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
                    const btn = event.target;
                    const cactusId = Number(btn.getAttribute('data-id'));
                    if (!confirm("ATENȚIE: Produsul va fi șters definitiv din baza de date. Continui?"))
                        return;
                    if (!confirm("Ești absolut sigur? Acțiunea este ireversibilă."))
                        return;
                    yield authFetch(`${API_BASE}/api/cacti/${cactusId}/permanent`, {
                        method: 'DELETE',
                    });
                    fetchAdminCacti();
                }));
...
if (addCactusBtn) {
    addCactusBtn.addEventListener('click', () => __awaiter(void 0, void 0, void 0, function* () {
        const newCactus = {
            ...
            imageUrl: document.getElementById('new-cactus-image').value.trim(),
        ...
        yield authFetch(`${API_BASE}/api/cacti`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(newCactus)
        });
        document.getElementById('new-cactus-name').value = "";
        document.getElementById('new-cactus-price').value = "";
        document.getElementById('new-cactus-desc').value = "";
        document.getElementById('new-cactus-image').value = "";
        document.getElementById('new-cactus-stock').value = "";
        fetchAdminCacti();
    }));
}
...
function fetchAdminOrders() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield authFetch(`${API_BASE}/api/orders`, {});
            if (!response.ok)
            ...
            document.querySelectorAll('.order-status-select').forEach(select => {
                select.addEventListener('change', (e) => __awaiter(this, void 0, void 0, function* () {
                    const target = e.target;
                    const orderId = target.getAttribute('data-id');
                    const newStatus = target.value;
                    const response = yield authFetch(`${API_BASE}/api/orders/${orderId}/status`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
...
function fetchPendingReviews() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!container)
            return;
        try {
            const response = yield authFetch(`${API_BASE}/api/reviews/pending`);
        ...
        `).join("");
            document.querySelectorAll('.approve-review-btn').forEach(btn => {
                btn.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                    const id = e.target.getAttribute('data-id');
                    yield authFetch(`${API_BASE}/api/reviews/${id}/approve`, { method: 'PUT' });
                    fetchPendingReviews();
                }));
            });
            document.querySelectorAll('.reject-review-btn').forEach(btn => {
                btn.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (confirm("Esti sigur ca vrei sa respingi aceasta recenzie?")) {
                        yield authFetch(`${API_BASE}/api/reviews/${id}`, { method: 'DELETE' });
                        fetchPendingReviews();
                    }
                }));
            });
